...
const Modal = ({
    ...
    data => {
      return (
        <Item
          ...
          checked={selectedValues.includes(data.item.value)}
          renderOption={renderOption}
        />
      );
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [customStyles, selectedValues, onSelect, renderOption],
  );
  ...
  }, [itemHeight, filterResult, renderItem]);
  ...
  const searchComponent = useMemo(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchable, keywords, customStyles.searchBox, customStyles.searchInput]);
